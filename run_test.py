...
# ---------------------------
def visualize_prediction(input_seq: torch.Tensor, gt: torch.Tensor, pred: torch.Tensor, crop: float = 0.0):
    """
    Displays:
      - Input triplet: t-1, t0, t+1
      - Ground truth
      - Prediction
      - Disparity maps:
          * GT - t0
          * GT - Prediction
    Includes MAE metrics in titles.
    """
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    # Disparities
    disparity_input = gt_np - seq_np[1]/1000
    disparity_pred  = gt_np - pred_np

    # MAEs
    mae_input = compute_mae(seq_np[1]/1000, gt_np)
    ...
    print(f"Shapes: {[img.shape for img in images]}")
...
if __name__ == "__main__":
    ...
    # create_sequence_gif(model, viz_loader, device, gif_path='saved_data_8/vis.gif', fps=8, crop=0.0)
    ...
    results = evaluate_sequence(
        ...
        crop=0.1,
        mask_value=0.0,
        visualize_indices=[0, 10, 20]  # or whatever you want to visualize
    )
